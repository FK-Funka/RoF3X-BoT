...
async function __START() {
    ...
    const sock = sockConnect({
        ...
        browser: ['RoF3X-MD','Safari','1.0.0'],
        auth: state,
        version
    })

store.bind(sock.ev)
console.log(color(figlet.textSync('R O F 3 X', {
...
console.log(color('Bot :', 'cyan'), color('RoF3X', 'yellow'))
console.log(color('Author :', 'cyan') color('FxSx', 'yellow'))
console.log(color('library :', 'cyan') color('Baileys', 'yellow'))
console.log(color('laguage :', 'cyan') color('JavaScript', 'yellow'))
console.log(color('\n', 'cyan'), color('\nWelcome back, Owner! Hope you are doing well~', 'yellow'))

sock.browserDescription = ["RoF3X-MD", "Mac", "3.0.0"];
...
let buttonMessage = {
image: fs.readFileSync("./worker/storage/hasil.jpg"),
caption: mbc,
...
const jamny = moment.tz('Asia/Jakarta').format('HH:mm:ss')
			let d = new Date
				...
				exec(`magick '../worker/media/image/lev.jpg' -gravity west -fill '#ffffff' -font '../worker/media/font/font-gue.ttf' -size 1280x710 -pointsize 75 -interline-spacing 7.5 -annotate +460-47 '${resa}' -pointsize 51 -annotate +460+83 '${jamny} ${calender}' -pointsize 48 -annotate +100+230 'MEMBER - ${behys}' -pointsize 63 -annotate +460+200 'Leaving from ${metadata.subject}' '${ppuser}' -resize %[fx:t?u.w*0.2:u.w]x%[fx:?u.h*0.2:u.h] -gravity center -geometry -430+66 -composite './worker/storage/hasild.jpg'`)
				.on('error', () => sock.sendMessage(m.chat, {text:'error'}))
				.on('exit', () => {
                    sock.sendMessage(anu.id, { image: fs.readFileSync("./worker/storage/hasild.jpg"), contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
                })
                }
            }
        } catch (err) {
