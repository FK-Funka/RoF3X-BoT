...
async function __START() {
    ...
    console.log('\n')
    console.log(color('•<><><>[  R O F 3 X  ]<><><>•', 'magenta'))
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    ...
                    orgnye = num
                    sendNy = fs.readFileSync('worker/media/audio/welcom.mp3')
                    npc = `Welcome @${orgnye.split("@")[0]}`
                    sock.sendMessage(m.chat, {audio: sendNy, mimetype:"audio/mp4", ptt:true, contextInfo:{externalAdReply:{
                    title: npc,
                    body: `${jm}`,
                    thumbnail: ppuser,
                    mentions:[orgnye],
                    ...
                    const jm = moment.tz('Asia/Jakarta').format('HH:mm:ss')
                    orgnye = num
                    mbc = `Selamat Tinggal @${orgnye.split("@")[0]}`
                    let buttons = [
                       {buttonId: `k`, buttonText: {displayText: 'Beyy Beban'}, type: 1}
                    ]
                    let buttonMessage = {
                    image: ppuser,
                    caption: npc,
